...
def load_all_aedit(root_dir, style):
  #Modifying this function to only include ibug, as it appears to have the most similar geometry to aedit
  mat_dir = osp.join(root_dir, 'Bounding_Boxes')
  pairs = [(osp.join(mat_dir,  'bounding_boxes_ibug.mat'), osp.join(root_dir, 'aedit-' + style, 'ibug'))]
...
def generage_aedit_list(root, save_dir, box_data, SUFFIX):
  ...
  with open(osp.join(save_dir, 'aedit.test.common.' + SUFFIX), 'w') as txtfile:
  ...
  with open(osp.join(save_dir, 'aedit.test.challenge.' + SUFFIX), 'w') as txtfile:
  ...
  with open(osp.join(save_dir, 'aedit.test.full.' + SUFFIX), 'w') as txtfile:
    for cpair in common_set:
      box_str = return_box(cpair[0], cpair[1], box_data, SUFFIX)
      txtfile.write('{} {} {}\n'.format(cpair[0], cpair[1], box_str))
      all_lines.append('{} {} {}\n'.format(cpair[0], cpair[1], box_str))
  ...
  with open(osp.join(save_dir, 'aedit.all.' + SUFFIX), 'w') as txtfile:
...
if __name__ == '__main__':
  ...
  path_aedit = Path.home() / 'datasets' / 'aedit-Style'
  print ('aedit Dir : {:}'.format(path_aedit))
  assert path_aedit.exists(), '{:} does not exists'.format(path_aedit)
  path_aedit = str(path_aedit)
  styles = ['female', 'male']
  USE_BOXES = ['GTB', 'DET']
  for USE_BOX in USE_BOXES:
    for style in styles:
      box_datas = load_all_aedit(path_aedit, style)
      print("box_datas \n")
      print(box_datas)
      SAVE_DIR = osp.join(this_dir, 'lists', 'aedit', style)
      Data_DIR = osp.join(path_aedit, 'aedit-' + style)
      generage_aedit_list(Data_DIR, SAVE_DIR, box_datas, USE_BOX)
